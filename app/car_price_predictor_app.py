...
@st.cache_resource    
def load_model():
    with open(r"C:\Users\dell\Documents\Research_Engineer\CarPricePrediction\saved_models\RandomForestRegressor.pkl", 'rb') as f:
        model = pickle.load(f)

    with open(r"C:\Users\dell\Documents\Research_Engineer\CarPricePrediction\saved_scaling\scaler.pkl", 'rb') as f:
...
with col2:
    ...
    seats = st.number_input("Seats", min_value=1, max_value=7, step=1)
...
if st.button("Predict Price", use_container_width=True):
    ...
    try:
        ...
        st.success(
            f"💰 Predicted Price: Rs. {format_values(prediction)}"
        )
